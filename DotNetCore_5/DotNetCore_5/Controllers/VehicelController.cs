...
namespace DotNetCore_5.Controllers
{
    ...
    public class VehicelController : Controller
    {
        ...
        {
            var buildings = _iVehicleRepository.GetAll();
            return Json(buildings, new JsonSerializerOptions());

        }
        [HttpPost]

        public JsonResult Create([FromBody]VehicleViewModel objModel)
        {
            if (ModelState.IsValid)
            {

                _iVehicleRepository.Save(objModel);
            }
            var data = _iVehicleRepository.GetAll();
        ...
        [HttpPost]
        public JsonResult Edit([FromBody] VehicleViewModel objModel)
        {
            if (ModelState.IsValid)
            {
                if (objModel.VehicleID > 0)
                {
                    _iVehicleRepository.Update(objModel);
                }
            }
            var data = _iVehicleRepository.GetAll();
            return Json(data, new JsonSerializerOptions());
